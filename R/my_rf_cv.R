#' generate a random forest cross-validation
#'
...
#'
#'
#' @return A \a numeric with the cross-validation error
#'
#' @keywords inference
#' @keywords prediction
#'
...
my_rf_cv <- function(k) {
  ...
  for(i in 1:k) {
    # select train data
    fold_train <- penguins_clean %>% filter(fold != i)

    # select test data
    fold_test <- penguins_clean %>% filter(fold == i)
